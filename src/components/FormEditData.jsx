...
function FormEditData() {
  ...
  useEffect(() => {
    const getData = async () => {
      try {
        const response = await axios.get(`http://localhost/be/read_by_id.php/${id}`);
  ...
  const updateMahasiswa = async (e) => {
    e.preventDefault();
    try {
      await axios.put(`http://localhost/be/update.php/${id}`, {
        npm: npm,
        nama: nama,
        kelas: kelas,
      });
  ...
  return (
    <div className="flex w-full justify-center items-center">
      ...
      <div className="p-4 lg:w-1/2">
        <form onSubmit={updateMahasiswa} className="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4">
          <div className="mb-4">
